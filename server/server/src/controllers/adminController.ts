...
export async function approveRegistrationRequest(req: Request, res: Response) {
  try {
    ...
    // Create user record, including password hash if provided (local registrations)
    const userData: any = {
      username,
      email,
      role: 'user',
    ...
    if (request.password_hash) {
      userData.password_hash = request.password_hash;
    }

    const newUser = await jsonStore.insert('users', userData);
