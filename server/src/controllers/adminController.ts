...
import { sendInvitationEmail, sendPasswordResetEmail, sendRegistrationVerificationEmail, sendRegistrationRequestEmail, sendEmail } from '../services/emailService';
...
export async function approveRegistrationRequest(req: Request, res: Response) {
  try {
    ...
    if (DB_AVAILABLE) {
      ...
      // Notify user with styled approval email (sendEmail imported at top)
      try {
    ...
    // Create user record; include password hash from request if available
    ...
    if (request.password_hash) {
      newUserData.password_hash = request.password_hash;
    }
    const newUser = await jsonStore.insert('users', newUserData);
    ...
    // Send styled notification email (JSON fallback) using imported sendEmail
    try {
      const signInUrl = `${process.env.FRONTEND_URL || 'http://localhost:5173'}${process.env.FRONTEND_BASE_PATH || '/journey'}/login`;
      const html = `
        <!DOCTYPE html>
...
export async function rejectRegistrationRequest(req: Request, res: Response) {
  try {
    ...
    if (DB_AVAILABLE) {
      ...
      // Optionally notify user using imported sendEmail
      try {
        await sendEmail({
          to: request.email,
    ...
    // Notify user using imported sendEmail
    try {
      await sendEmail({
        to: request.email,
        subject: 'Your registration request was rejected',
