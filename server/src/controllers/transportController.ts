...
async function assertWithinJourney(journeyId: string, start: Date, end: Date) {
  if (DB_AVAILABLE) {
...
export const deleteTransport = async (req: Request, res: Response) => {
  try {
    ...
    if (!DB_AVAILABLE) {
      ...
      // also delete any transport-specific attachments when using JSON store
      await Promise.all(((await jsonStore.findByField('transport_attachments','transport_id',transportId))||[]).map((a:any)=>jsonStore.deleteById('transport_attachments', a.id)));
